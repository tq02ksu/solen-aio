...
public class CoordinateTransformationController {
     ...
     * @return bd09 coordinates
     */
    @RequestMapping("/gcj02-to-bd09")
    public List<Coordinate> gcj02ToBd09(@RequestBody List<Coordinate> coords) {
        return coords.stream().map(c -> {
            ...
            return Coordinate.builder().lat(bdLat).lng(bdLng).build();
    ...
    public List<Coordinate> bd09ToGcj02(@RequestBody List<Coordinate> coords) {
        return coords.stream().map(c -> {
            ...
            return Coordinate.builder().lng(lng).lat(lat).build();
     ...
     * WGS84转GCJ02(火星坐标系)
     *
     * @param coords coordinates need to process
    ...
    public List<Coordinate> wgs84ToGcj02(@RequestBody List<Coordinate> coords) {
        return coords.stream().map(c -> {
            ...
            return Coordinate.builder().lat(mgLat).lng(mgLng).build();
        }).collect(Collectors.toList());
    }
    ...
    public List<Coordinate> gcj02ToWgs84(@RequestBody List<Coordinate> coords) {
        return coords.stream().map(c -> {
            ...
            double mgLat = c.getLat() + dLat;
            double mgLng = c.getLng() + dLng;
            return Coordinate.builder().lng(c.getLng() * 2 - mgLng).lat(c.getLat() * 2 - mgLat).build();
        }).collect(Collectors.toList());
    }
